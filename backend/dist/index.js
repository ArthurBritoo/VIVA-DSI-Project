...
if (admin.apps.length > 0) {
    ...
    console.log("Default Firebase Admin SDK app options (partial):", {
        ...
        credentialType: defaultApp.options.credential?.type, // Access type property of credential
        clientEmail: defaultApp.options.credential?.clientEmail,
    });
}
// *** ADDED LOGS END ***
...
app.post("/anuncios", authenticate, async (req, res) => {
    ...
    try {
        ...
        const newAnuncio = await (0, AnuncioController_1.createAnuncio)(db, anuncioData, userId);
...
app.get("/anuncios", authenticate, async (req, res) => {
    try {
        ...
        const anuncios = await (0, AnuncioController_1.getAnuncios)(db);
...
app.get("/anuncios/:id", async (req, res) => {
    try {
        ...
        const anuncio = await (0, AnuncioController_1.getAnuncioById)(db, id);
        if (anuncio) {
...
app.put("/anuncios/:id", authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await (0, AnuncioController_1.updateAnuncio)(db, id, updatedData);
...
app.delete("/anuncios/:id", authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.uid; // Get userId from authenticated user
        if (!userId) {
        ...
        // You might want to add logic here to ensure the authenticated user is the owner of the anuncio
        await (0, AnuncioController_1.deleteAnuncio)(db, id);
...
app.get("/anuncios/user/:userId", authenticate, async (req, res) => {
    try {
        const { userId } = req.params;
        // Optional: Add a check to ensure req.user.uid === userId for security
        const anuncios = await (0, AnuncioController_1.getAnunciosByUserId)(db, userId);
        res.status(200).send(anuncios);
    }
    catch (error) {
